        ...
        perSystem =
          ...
          let
            runtimeInputs = with pkgs; [
              inputs'.esp.packages.esp-idf-full
          ...
          in
          {
            packages =
                  ...
                  pkgs.runCommand "faikin-${name}"
                    ...
                    ''
                      ...
                      if ! [ -e ESP32-BLE-Env ]; then
                        >&2 echo "Missing submodules. You need to specify ?submodules=1 when invoking this flake"
                        >&2 echo "(e.g. \`nix build 'git+https://github.com/MaienM/ESP32-Faikin?submodules=1#${name}'\`)."
                        exit 1
                      fi
                      ...
                      mkdir -p components/
                      ${toString (
                        pkgs.lib.attrsets.mapAttrsToList (name: path: ''
              ...
              builtins.mapAttrs (
                name: suffix:
                let
                  withConfig = mkPackage name suffix;
                in
                (withConfig { }) // { inherit withConfig; }
              ) targets;
